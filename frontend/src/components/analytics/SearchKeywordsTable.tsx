				...
				<table className="w-full text-sm">
					<thead>
						<tr className="border-b border-sand-300">
							<th className="text-left text-sand-700 py-2 pr-4">#</th>
							<th className="text-left text-sand-700 py-2 pr-4">키워드</th>
							<th className="text-right text-sand-700 py-2 pr-4">클릭</th>
							<th className="text-right text-sand-700 py-2 pr-4">노출</th>
							<th className="text-right text-sand-700 py-2 pr-4">CTR</th>
							<th className="text-right text-sand-700 py-2">순위</th>
						</tr>
					</thead>
					<tbody>
						{keywords.length === 0 ? (
							<tr>
								<td colSpan={6} className="text-center text-sand-600 py-8">
									검색 데이터가 없습니다
								</td>
							</tr>
						) : (
							keywords.slice(0, 15).map((kw, i) => (
								<tr key={kw.query} className="border-b border-sand-100 hover:bg-sand-50">
									<td className="py-2 pr-4 text-sand-600">{i + 1}</td>
									...
									<td className="py-2 pr-4 text-right text-sand-700">{kw.impressions.toLocaleString()}</td>
