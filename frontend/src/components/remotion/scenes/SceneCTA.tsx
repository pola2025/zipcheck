...
export function SceneCTA({ variant = 'mobile' }: SceneProps) {
	...
	return (
		...
		>
			...
			>
				<p className={`text-forest-200 mb-3 ${isDesktop ? 'text-base' : 'text-sm'}`}>인테리어 유통망 원가 기반</p>
				<h2 className={`text-white font-bold mb-2 ${isDesktop ? 'text-4xl' : 'text-3xl'}`}>
					내 견적서, 적정가인지<br />확인해보세요
				</h2>
				<p className={`text-forest-200 mb-8 ${isDesktop ? 'text-base' : 'text-sm'}`}>업로드만 하면 항목별 리스크를 분석해드립니다</p>

				<div style={{ transform: `scale(${buttonPulse})` }}>
					<div className={`inline-block bg-white text-forest-700 font-bold rounded-full shadow-lg ${isDesktop ? 'px-10 py-4 text-lg' : 'px-8 py-3.5 text-base'}`}>
						내 견적서 분석받기 &rarr;
					</div>
				</div>
			</div>
