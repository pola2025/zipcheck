...
import { FileText, Play, Eye, Filter, RefreshCw, Search, AlertTriangle, Trash2 } from 'lucide-react'
...
export default function QuoteRequests() {
	...
	const runAutoAnalysis = async (id: string) => {
	...
	// Extract score from analysis_result for completed items
	const getScore = (req: QuoteRequest): number | null => {
		if (req.status !== 'completed' || !req.analysis_result) return null
		const result = req.analysis_result as Record<string, unknown>
		return (result.final_score as number) ?? null
	}
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-sm text-red-600">
								{requests.filter(r => r.validation_status === 'rejected_insufficient_detail').length}건
							</p>
						</div>
					</div>
				</motion.div>
			...
			<div className="bg-white rounded-2xl p-5 border border-sand-300">
				...
				<div className="mt-4 flex items-center justify-between">
					<div className="text-sm text-sand-700">
						총 <span className="text-forest-600 font-semibold">{filteredRequests.length}</span>건
					</div>
					<button
						...
						<table className="w-full">
							<thead>
								<tr className="border-b border-sand-300 bg-sand-50">
									<th className="text-left py-4 px-4 text-sm font-semibold text-sand-700">신청일시</th>
									<th className="text-left py-4 px-4 text-sm font-semibold text-sand-700">고객 정보</th>
									<th className="text-left py-4 px-4 text-sm font-semibold text-sand-700">매물 정보</th>
									<th className="text-left py-4 px-4 text-sm font-semibold text-sand-700">항목</th>
									<th className="text-left py-4 px-4 text-sm font-semibold text-sand-700">상태</th>
									<th className="text-left py-4 px-4 text-sm font-semibold text-sand-700">점수</th>
									<th className="text-left py-4 px-4 text-sm font-semibold text-sand-700">작업</th>
								</tr>
							</thead>
							<tbody>
								{filteredRequests.map((request, index) => {
									const score = getScore(request)
									...
									return (
										<>
											...
											>
												...
												<td className="py-4 px-4">
													<div className="text-sm">
														...
														<div className="text-sand-600 text-xs">{request.customer_phone}</div>
													</div>
												</td>
												<td className="py-4 px-4">
													<div className="text-sm">
														<div className="text-sand-900">
															...
															{request.property_size && (
																<span className="text-sand-600 text-xs ml-1">
																	{request.property_size}㎡ ({(request.property_size / 3.3058).toFixed(0)}평)
																</span>
															)}
														</div>
												...
												<td className="py-4 px-4">
													{score !== null ? (
														<span className={`px-3 py-1 rounded-full text-xs font-bold border ${gradeColors[gradeLabel]}`}>
															{gradeLabel} · {score}점
														</span>
													) : (
												...
												<td className="py-4 px-4" onClick={(e) => e.stopPropagation()}>
													<div className="flex gap-1.5">
														...
														{request.status === 'pending' && (
															<button
																onClick={() => runAutoAnalysis(request.id)}
											...
											{expandedId === request.id && (
												<tr key={`${request.id}-expanded`} className="bg-sand-50">
													<td colSpan={7} className="p-6">
														...
														>
															...
															<div className="bg-white rounded-xl p-4 border border-sand-300">
																<h4 className="text-sm font-semibold text-sand-700 mb-3">견적 항목 ({request.items?.length || 0}개)</h4>
																<div className="space-y-2 max-h-64 overflow-y-auto">
																	{(request.items || []).map((rawItem, idx) => {
																			...
																			<div key={idx} className="bg-sand-50 rounded-xl p-3 text-sm border border-sand-100">
																				<div className="flex justify-between items-start">
																					<span className="font-semibold text-forest-600">
																						{(item.category as string) || '카테고리 없음'}
																					</span>
																					<span className="text-wood-500 font-bold">
																						{((item.quoted_price || item.totalPrice || item.total_price) as number)?.toLocaleString()}원
																					</span>
																				</div>
																				{((item.item_name as string) || (item.itemName as string)) && (
																					<div className="text-sand-700 mt-1">
				...
				>
					...
					<div className="text-sm text-amber-700">
						...
						<p className="text-amber-600">
							<Play className="w-3.5 h-3.5 inline-block mr-1" />
							버튼을 클릭하여 자동 분석을 실행하세요. 분석 결과가 즉시 생성됩니다.
						</p>
					</div>
				</motion.div>
			)}
